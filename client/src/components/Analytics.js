...
const Analytics = ({ allTransection }) => {
  ...
  const totalExpenseTransections = allTransection.filter(
    (transaction) => transaction.type === "expense"
  );
  const totalIncomePercent =
    (totalIncomeTransections.length / totalTransaction) * 100;
  const totalExpensePercent =
    (totalExpenseTransections.length / totalTransaction) * 100;
